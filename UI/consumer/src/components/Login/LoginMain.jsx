...
const LoginWithGoogleButton = () => {
  ...
  const {isLoggedIn, handleLoginLocal} = useContext(loginContext);
  useEffect(()=>{
    if(isLoggedIn){
      navigate('/');
    }
  ...
  const handleLogin = useGoogleLogin({
    onSuccess: async tokenResponse => {
      ...
      const response = await axios.post(`${API_URL}/api/v1/consumer/auth/google`, {
          access_token: tokenResponse.access_token,  // Send access token to backend
        });
        if (response) {
